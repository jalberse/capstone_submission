...
text = os.linesep.join([s for s in text.splitlines() if (s and len(s) > 10)])

# For now, just remove a ton of tweets because I haven't implemented batching yet
# we run into memory issues training on larger dataset
line_list = [s for s in text.splitlines() if (s and len(s) > 10)]
text = os.linesep.join(line_list[:len(line_list)//100])
